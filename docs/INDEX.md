# Self-Storage Aggregator Platform - Documentation Index
# Generated: 2026-02-26 | ✅ Migration Complete (Phase 0-3)

## Migration Status

✅ **Migration Complete:** All documents adapted for UAE market (Phase 0-3 complete)
✅ **Market:** United Arab Emirates
✅ **Domain:** storagecompare.ae
✅ **Legal Framework:** UAE PDPL, Civil Transactions Law, TDRA
✅ **Services:** Google Maps, Stripe, Twilio + WhatsApp

## Document Organization Overview

This index organizes all project documentation into 9 tiers, from core specifications to supporting documents.

**Phase 0.5 Update:** All 115 documents have been classified and organized.
**Phase 1 Update:** Automated RU→UAE replacements applied (110 docs)
**Phase 2 Update:** Document-by-document contextual adaptation complete (all 8 tiers)
**Phase 3 Update:** Cross-reference validation complete

**Unsorted:** 1 file (NEEDS_REVIEW.md only)
**Archived:** 5 older document versions

---

## Summary Statistics

| Tier | Category | Count |
|------|----------|-------|
| 1 | Core Specifications | 9 |
| 2 | Security & Non-Functional | 8 |
| 3 | Feature Specifications | 31 |
| 4 | Frontend & UX | 17 |
| 5 | Infrastructure & Operations | 12 |
| 6 | Legal & Compliance | 6 |
| 7 | Multi-Region & Scaling | 3 |
| 8 | Operations & Supporting | 23 |
| - | Data & Glossary | 0 |
| - | Archived Versions | 5 |
| - | Unsorted / Needs Review | 1 |

---

## TIER 1: Core Specifications

**Canonical source of truth - highest priority**

- **API_Detailed_Specification_MVP_v1_COMPLETE.md**
  - API Detailed Specification (MVP v1) - COMPLETE
- **DOC-022 Backend_implementation_plan_mvp_v1_CANONICAL.md**
  - Backend Implementation Plan (MVP v1) — Self-Storage Aggregator
- **DOC-059_Multi-Country_Multi-Region_Technical_Architecture_MVP_v1_1_HARDENED.md**
  - DOC-059 — Multi-Country & Multi-Region Technical Architecture (MVP v1)
- **DOC-101_Internal_Admin_API_Specification_MVP_v1.md**
  - Internal Admin API Specification (MVP v1)
- **full_database_specification_mvp_v1_CANONICAL.md**
  - Full Database Specification for Self-Storage Aggregator MVP v1 — CANONICAL
- **Functional_Specification_MVP_v1_CORRECTED.md**
  - Functional Specification MVP v1 — Corrected Version
- **High_Level_Technical_Architecture_MVP_v1.md**
  - High-Level Technical Architecture (MVP v1)
- **MVP_Requirements_Specification_Full_TZ_v1.1_GREEN.md**
  - MVP Requirements Specification (Full TZ)
- **Technical_Architecture_Document_MVP_v1_CANONICAL.md**
  - Technical Architecture Document

---

## TIER 2: Security & Non-Functional Requirements

- **API_Rate_Limiting_Throttling_Specification_MVP_v1_CANONICAL.md**
  - **API RATE LIMITING & THROTTLING SPECIFICATION**
- **DOC-020 Аudit_logging_specification_mvp_v1_CANONICAL.md**
  - Audit Logging Specification (MVP v1)
- **DOC-078_Security_and_Compliance_Plan_MVP_v1_CANONICAL_REVISED.md**
  - Security and Compliance Plan - MVP v1
- **DOC-093_Warehouse_Safety_Security_Scoring_Engine_v2.md**
  - Warehouse Safety & Security Scoring Engine (Post-MVP v2)
- **DOC-102_Service_to_Service_Auth_Secrets_Management_MVP_v1_CANONICAL.md**
  - Service-to-Service Authentication & Secrets Management Specification
- **Error_Handling_Fault_Tolerance_Specification_MVP_v1_CANONICAL.md**
  - Error Handling & Fault Tolerance Specification (MVP v1) - CANONICAL
- **Logging_Strategy_CANONICAL_Contract_v2.md**
  - Logging Strategy & Log Taxonomy (MVP v1)
- **Security_Architecture_MVP_v1_CANONICAL.md**
  - Security Architecture (MVP v1)

---

## TIER 3: Feature Specifications

- **AB_Testing_Framework_FULL_CORRECTED.md**
  - A/B Testing Framework — Deep Technical Specification (MVP → v2)
- **AI_Core_Design_MVP_v1_CANONICAL.md**
  - AI Core Design (MVP v1)
- **Analytics_Metrics_Tracking_Specification_MVP_v1_1.md**
  - Analytics, Metrics & Tracking Specification
- **CRM_Lead_Management_System_MVP_v1_CANONICAL .md**
  - CRM Lead Management System (MVP v1) — CANONICAL
- **Data_Governance_DQ_Specification_COMPLETE.md**
  - Data Governance & Data Quality Specification (MVP → v2)
- **DOC-007 AI_Chat_Assistant_MVP_v1_HARDENED.md**
  - AI Chat Assistant (RAG + LLM) — Deep Technical Specification (MVP v1)
- **DOC-011 Pricing_Recommendation_Engine_MVP_v1.md**
  - Pricing Recommendation Engine — MVP v1 (Technical Scope Specification)
- **DOC-012_AI_Assisted_Content_Generator_MVP_v1.md**
  - DOC-012: AI-Assisted Warehouse Content Generator — Deep Technical Specificatio
- **DOC-013_AI_Anomaly_Detection_Prices_Demand_MVP_v2.md**
  - DOC-013: AI-Based Anomaly Detection for Prices & Demand
- **DOC-021_Operator_Risk_Signals_Manual_Review_Framework_MVP_v1.md**
  - Operator Risk Signals & Manual Review Framework (MVP v1)
- **DOC-023 Booking_Flow_Technical_Specification.md**
  - Booking Flow — Supporting Technical Specification (Reference Document)
- **DOC-024_Box_Size_Recommendation_Engine_MVP_v1.md**
  - DOC-024: Box Size Recommendation Engine — Deep Specification (MVP v1)
- **DOC-025_Box_Type_Classification_Engine_AI_Deep_Tech_Spec_MVP_v2.md**
  - DOC-025 — Box Type Classification Engine (AI) — Deep Technical Specification
- **DOC-038_Demand_Forecasting_Engine_Deep_Tech_Spec_v2.md**
  - DOC-038: Demand Forecasting Engine — Deep Tech Specification (MVP → v2)
- **DOC-053_Lead_Scoring_AI_Engine_Deep_Spec_MVP_to_v2_HARDENED.md**
  - DOC-053: Lead Scoring AI Engine — Deep Tech Specification (MVP → v2)
- **DOC-061_Operator_Billing_Settlement_System_Deep_Spec_MVP_v2.md**
  - Operator Billing & Settlement System — Deep Tech Specification (MVP → v2)
- **DOC-064_Operator_Manual_Pricing_vs_AI_Dynamic_Pricing_Specification_MVP_v2.md**
  - DOC-064: Operator Manual Pricing vs AI Dynamic Pricing Specification (MVP → v2
- **DOC-065_Operator_Staff_Management_Multi_User_Roles_MVP_v2.md**
  - DOC-065: Operator Staff Management & Multi-User Roles (MVP → v2)
- **DOC-066_Partner_API_Specification_MVP_v1_CANONICAL.md**
  - Partner API Specification (MVP v1)
- **DOC-068_Photo_Quality_Scoring_Engine_v1.md**
  - DOC-068: Photo Quality Scoring Engine — Deep Tech Specification
- **DOC-069_Price_Analytics_Engine_MVP_v1_HARDENED.md**
  - DOC-069: Price Analytics Engine — Deep Specification (MVP v1)
- **DOC-075 Search_Ranking_Algorithm_Specification_MVP_v2.md**
  - DOC-075 — Search Ranking Algorithm — Supporting / Reference Specification (v
- **DOC-076_Search_Ranking_Algorithm_v2_Advanced.md**
  - Search Ranking Algorithm v2 (Advanced)
- **DOC-092_Warehouse_Quality_Score_Algorithm_v2.md**
  - DOC-092: Warehouse Quality Score Algorithm (AI, v2)
- **DOC-096 Operator_Onboarding_Specification_COMPLETE.md**
  - Operator Onboarding & Verification Specification
- **DOC-097_Payment_Billing_Integration_Architecture_v2.md**
  - Payment & Billing Integration Architecture (v2)
- **DOC-099_Content_Moderation_Fraud_Protection_Policy_MVP_v2.md**
  - DOC-099 — Content Moderation & Fraud Protection Policy
- **DOC-100_Caching_Strategy_CDN_Specification_MVP_v1.md**
  - DOC-100: Caching Strategy & CDN Specification (MVP v1)
- **DOC-104_Search_Query_Normalization_Expansion_Specification_MVP_v1.md**
  - DOC-104 — Search Query Normalization & Expansion Specification
- **DOC-105_AI_Risk_Scoring_Fraud_Detection_Engine_MVP_v1.md**
  - AI Risk Scoring & Fraud Detection Engine
- **DOC-106_Trust_Safety_Framework_MVP_v1.md**
  - Trust & Safety Framework

---

## TIER 4: Frontend & UX

- **Admin_Panel_UX_Specification_MVP_v1_CORRECTED.md**
  - Admin Panel — UX/UI Specification (MVP v1)
- **Design_System_Overview_MVP_v1.md**
  - Design System Overview (MVP v1)
- **DOC-004 Accessibility_Guidelines_MVP_v1_CANONICAL.md**
  - Accessibility Guidelines (MVP v1)
- **DOC-026_Branding_Visual_Identity_Guidelines_MVP_v1.md**
  - Branding & Visual Identity Guidelines (MVP v1)
- **DOC-048_Frontend_Performance_Optimization_Plan_MVP_v1.md**
  - DOC-048 — Frontend Performance Optimization Plan (MVP v1)
- **DOC-049_Frontend_SEO_Strategy_MVP_v1.md**
  - DOC-049: Frontend SEO Strategy (MVP v1)
- **DOC-056_Mobile_App_UX_Specification_MVP_to_v2_TIGHTENED.md**
  - Mobile App UX Specification (MVP → v2)
- **DOC-063_Operator_Experience_OX_Deep_Specification_MVP_v1_INDEX_HARDENED.md**
  - Operator Experience (OX) — Deep Specification (MVP v1)
- **DOC-077_Search_UX_Filters_Behaviour_Specification_v2.md**
  - Search UX & Filters Behaviour Specification (v2)
- **DOC-081_SEO_Strategy_MVP_v1_CANONICAL.md**
  - DOC-081 — SEO Strategy (MVP v1) — CANONICAL
- **DOC-082_SEO_Strategy_Technical_SEO_Specification_v2.md**
  - DOC-082 — SEO Strategy & Technical SEO Specification (v2)
- **DOC-088_User_Experience_Renters_Deep_Specification.md**
  - User Experience (Renters) — Deep Specification
- **DOC-089 User_Personas_and_User_Journeys_MVP_v1.md**
  - User Personas & User Journeys (MVP v1)
- **DOC-091_UX_Flow_Diagrams_Wireframes_Specification_MVP_v1.md**
  - UX Flow Diagrams & Wireframes Specification (MVP v1)
- **Frontend_Architecture_Specification_v1.5_FINAL.md**
  - Frontend Architecture Specification (MVP v1)
- **frontend_implementation_plan_mvp_v1_AUDITED.md**
  - Frontend Implementation Plan (MVP v1) — Self-Storage Aggregator
- **Operator_Dashboard_Deep_Specification_v1.0_FINAL.md**
  - Operator Dashboard Deep Specification

---

## TIER 5: Infrastructure & Operations

- **Configuration_Management_Strategy_MVP_v1.md**
  - Configuration Management Strategy (MVP v1)
- **DOC-010 AI_Model_Monitoring_Versioning_MVP_v1_REVISED.md**
  - AI Model Monitoring & Versioning (MVP v1)
- **DOC-027_Capacity_Planning_Scaling_Roadmap_MVP_v1_CANONICAL.md**
  - DOC-027: Capacity Planning & Scaling Roadmap
- **DOC-033_Cost_Optimization_Plan_Infrastructure_Cloud_MVP_v1_CANONICAL.md**
  - Cost Optimization Plan (Infrastructure & Cloud)
- **DOC-037_Data_Warehouse_BI_Planning_MVP_v2.md**
  - DOC-037: Data Warehouse & BI Planning (MVP → v2)
- **DOC-041 DevOps_Infrastructure_Plan_MVP_v1_CANONICAL.md**
  - DevOps Infrastructure Plan (MVP v1) — CANONICAL
- **DOC-042_Disaster_Recovery_Backup_Plan_MVP_v1_CANONICAL.md**
  - DISASTER RECOVERY & BACKUP PLAN (MVP v1)
- **DOC-043 Distributed_tracing_strategy_mvp_v1_CANONICAL.md**
  - Distributed Tracing Strategy (MVP v1)
- **DOC-052 Infrastructure_as_Code_Plan_MVP_v1_CANONICAL.md**
  - Infrastructure as Code (IaC) Plan
- **DOC-067 Performance_Load_Testing_Plan_MVP_v1_COMPLETE.md**
  - Performance & Load Testing Plan - MVP v1
- **DOC-083 SLO_SLA_SLI_Definitions_MVP_v1.md**
  - SLO / SLA / SLI Definitions (MVP v1)
- **Monitoring_and_Observability_Plan_MVP_v1_CANONICAL.md**
  - Monitoring & Observability Plan (MVP v1) — CANONICAL

---

## TIER 6: Legal & Compliance

- **Data_retention_policy_mvp_v1.md**
  - Data Retention Policy (MVP v1)
- **DOC-054_Legal_Checklist_Compliance_Requirements_MVP_v1_CANONICAL.md**
  - Legal Checklist & Compliance Requirements (MVP v1)
- **DOC-060_Operator_Agreement_MVP_v1_CANONICAL.md**
  - OPERATOR AGREEMENT (MVP v1)
- **DOC-071_Privacy_Data_Protection_Policy_Technical_MVP_v1.md**
  - DOC-071: Privacy & Data Protection Policy — Technical Version (MVP v1)
- **DOC-094_Legal_Operational_Model_Russia_MVP_v1.md**
  - DOC-094 — Legal & Operational Model for Russia (MVP v1)
- **Legal_Documentation_Unified_Guide_MVP_v1.md**
  - Legal Documentation Unified Guide (MVP v1)

---

## TIER 7: Multi-Region & Scaling

- **DOC-045_Feature_Roadmap_Release_Phases_CANONICAL_v1.md**
  - Feature Roadmap & Release Phases
- **DOC-058_Multi_Country_Multi_Region_Scaling_Specification_v1_1_HARDENED.md**
  - Multi-Country / Multi-Region Scaling Specification (Post-MVP)
- **DOC-095_Multi_Country_Multi_Region_Expansion_Strategy_Global_v1.md**
  - Multi-Country / Multi-Region Expansion Strategy (Global v1)

---

## TIER 8: Operations & Supporting

- **Architecture_review_checklist_mvp_v1.md**
  - Architecture Review Checklist (MVP v1)
- **DOC-000_Audit_Registry_v1_5_FINAL.md**
  - DOC-000: Audit Registry & Canonical Document Index
- **DOC-006 Admin_Runbooks_Pack_MVP_v1_CANONICAL.md**
  - Admin Runbooks Pack (MVP v1)
- **DOC-008_Competitor_Signals_Market_Insights_MVP_v1.md**
  - Competitor Signals & Market Insights (MVP v1)
- **DOC-018_API_Versioning_Deprecation_Policy_MVP_v1.md**
  - API Versioning & Deprecation Policy (MVP v1)
- **DOC-028_Change_Management_Process_MVP_v2.md**
  - Change Management Process (MVP → v2)
- **DOC-030_Competitive_Analysis_Market_Landscape_MVP_v1.md**
  - DOC-030 — Competitive Analysis & Market Landscape (MVP v1)
- **DOC-032_Content_Guidelines_for_Operators_MVP_v1.md**
  - DOC-032: Content Guidelines for Operators (MVP v1)
- **DOC-035 Data_migration_and_etl_plan_mvp_v1_HARDENED.md**
  - Data Migration & ETL Plan (MVP v1)
- **DOC-039_Deployment_Runbook_MVP_v1_1_REVISED.md**
  - Deployment Runbook - Self-Storage Aggregator MVP v1 (DOC-039)
- **DOC-070 Pricing_Monetization_Strategy_MVP_v1_FINAL_HARDENED.md**
  - Pricing & Monetization Strategy (MVP v1)
- **DOC-073 QA_Testing_Plan_MVP_v1_Complete_FIXED.md**
  - QA & Testing Plan (MVP v1)
- **DOC-074 Release_management_versioning_strategy_v1_1.md**
  - Release Management & Versioning Strategy (v1.1)
- **DOC-080_Self-Storage_Marketplace_Monetization_Strategy_v2.md**
  - DOC-080 — Self-Storage Marketplace Monetization Strategy (v2)
- **DOC-084 Support_maintenance_playbook_v1_1.md**
  - Support & Maintenance Playbook (v1.1)
- **DOC-085 Team_engineering_process_guidelines_mvp_v1_1.md**
  - Team & Engineering Process Guidelines (MVP v1)
- **DOC-087 User_Operator_Documentation_MVP_v1.md**
  - User & Operator Documentation (MVP v1)
- **DOC-090_User_Stories_Acceptance_Criteria_API_Mapping_MVP_v1.md**
  - DOC-090: User Stories + Acceptance Criteria + API Spec Mapping
- **DOC-103 Backup_and_Restore_Operational_Playbook_v1_1.md**
  - Backup & Restore Operational Playbook (v1.1)
- **DOC-107_Competitive_Analysis_Market_Landscape_UAE_v1.md**
  - DOC-107 — Competitive Analysis & Market Landscape (UAE Market v1)
- **DOC-108_Competitive_Analysis_Market_Landscape_Kazakhstan_v1.md**
  - DOC-108 — Competitive Analysis & Market Landscape (Kazakhstan Market v1)
- **Documentation_Map_v1_2_FINAL.md**
  - Documentation Map — Self-Storage Aggregator
- **incident_response_plan_mvp_v1_FULL.md**
  - INCIDENT RESPONSE PLAN (MVP v1)

---

## Data & Glossary

- **unified_data_dictionary_mvp_v1.csv**

---

## Archived Document Versions

These are older versions of documents that have been superseded by canonical versions:

- ADMIN-001_Correction_Summary.md
- backend_implementation_plan_mvp_v1_v2.0_NON_CANONICAL.md
- DOC-029_Change_Management_Process_MVP_v1.md
- security_and_compliance_plan_mvp_v1_CANONICAL_Dec15.md
- security_and_compliance_plan_mvp_v1_Russian.md

---

## Migration Complete - Next Steps

✅ **Phase 0 Complete:** All documents organized into proper tier structure (115 docs)
✅ **Phase 0.5 Complete:** All unsorted documents classified and duplicates archived
✅ **Phase 1 Complete:** Automated RU→UAE replacements applied (14 files modified)
✅ **Phase 2 Complete:** Document-by-document contextual UAE adaptation (all 8 tiers processed)
✅ **Phase 3 Complete:** Cross-reference validation complete

**Migration Results:**
- **Domain:** selfstorage.ru → storagecompare.ae
- **Legal Framework:** Russian laws (152-ФЗ, ГК РФ, Роскомнадзор) → UAE laws (PDPL, Civil Transactions Law, TDRA)
- **Services:** Yandex Maps → Google Maps, Yookassa → Stripe, SMSC → Twilio + WhatsApp
- **Market Adaptation:** All Russian examples → UAE examples (names, addresses, phone numbers, currency)
- **Critical Deliverable:** DOC-094 completely rewritten for UAE legal framework

**Documentation Ready for UAE Market Launch** 🚀

See PHASE1_RESULTS.md, PHASE2_RESULTS.md for detailed migration reports.

