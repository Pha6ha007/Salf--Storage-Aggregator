...
|---------------|----------|---------|
| Legal Entity (LLC, FZE) | Компании, официально зарегистрированные как юрлица | "StorageBiz LLC", "Складовая компания АО" |
...
- Проверка данных в публичных реестрах (UAE Commercial Registry, FTA (Federal Tax Authority))
...
- Интеграция с внешними API (FTA (Federal Tax Authority), банковские реквизиты)
...
- Загружает обязательные документы:
  - Свидетельство о регистрации юрлица / выписка из UAE Commercial Registry (для юрлиц)
...
- Выполняет анализ:
  - TRN Verification в базах FTA (Federal Tax Authority) (если доступно API)
...
- Выполняет дополнительные проверки:
  - TRN Verification на сайте FTA (Federal Tax Authority) вручную (fta.gov.ae)
...
**Для юридических лиц (LLC, FZE):**
- Свидетельство о регистрации или выписка из UAE Commercial Registry (не старше 3 месяцев)
- Устав (если требуется)
- Приказ о назначении директора
- Справка о банковских реквизитах или скрин из банк-клиента

**Для ИП:**
- Свидетельство о регистрации ИП или выписка из UAE Commercial Registry (не старше 3 месяцев)
...
3. Выполняет дополнительные проверки:
   - Проверяет ИНН на сайте FTA (Federal Tax Authority): https://egrul.fta.gov.ae/
...
| 5 | Тип оператора | select (dropdown) | Обязательный выбор | "Legal Entity (LLC, FZE)" | "Выберите тип вашего бизнеса" |
...
|----------|-------|-------------|
| `legal_entity` | Legal Entity (LLC, FZE) | Для компаний, зарегистрированных как LLC, FZE, FZCO |
...
| Валюта | Для ценообразования и расчетов | RUB (AED ) | BYN, KZT, USD |
...
|---------------|-----|-------------------|---------------|------------------------|
| Legal Entity (LLC, FZE) | `legal_entity` | Полностью зарегистрированная компания | Низкий | Полный пакет корп. документов |
...
### 1. Юридическое лицо (LLC, FZE, FZCO)

**Характеристики:**
- Официально зарегистрированная компания в UAE Commercial Registry
...
|---|----------|----------------|--------|--------------|
| 1 | Свидетельство о регистрации юрлица или Выписка из UAE Commercial Registry | ✅ Обязательно | PDF, JPG, PNG | Не старше 3 месяцев |
...
|------|----------|--------|
| Название компании | Полное наименование с ОПФ | StorageBiz LLC |
| ИНН | 10 цифр | 7701234567 |
| ОГРН | 13 цифр | 1027700000000 |
| КПП | 9 цифр | 770101001 |
| Юридический адрес | Полный адрес регистрации | 123456, Dubai, Al Quoz, д. 1, офис 10 |
...
- TRN Verification через контрольную сумму
- Проверка ОГРН через контрольную сумму
- Проверка БИК в справочнике БИК РФ
- Поиск компании в UAE Commercial Registry через API FTA (Federal Tax Authority) (если доступно)
...
**Характеристики:**
- Физическое лицо, зарегистрированное в UAE Commercial Registry
...
| № | Документ | Обязательность | Формат | Актуальность |
|---|----------|----------------|--------|--------------|
| 1 | Свидетельство о регистрации ИП или Выписка из UAE Commercial Registry | ✅ Обязательно | PDF, JPG, PNG | Не старше 3 месяцев |
...
| Адрес регистрации | Адрес прописки из паспорта | 123456, Dubai, Al Quoz, д. 1, кв. 10 |
...
**Автоматические проверки:**
- TRN Verification через контрольную сумму (12 цифр)
- Проверка ОГРНИП через контрольную сумму
- Проверка БИК в справочнике БИК РФ
- Поиск ИП в UAE Commercial Registry через API FTA (Federal Tax Authority) (если доступно)
...
- Проверка прописки (должна совпадать с адресом в выписке UAE Commercial Registry)
...
- Нет регистрации в UAE Commercial Registry
...
- Проверка статуса самозанятого через API FTA (Federal Tax Authority) (если доступно)
...
| Адрес помещения | Адрес склада | 123456, Dubai, Al Quoz, д. 1 |
...
|----------|--------|----|-----------|----|
| Выписка из реестра | ✅ UAE Commercial Registry | ✅ UAE Commercial Registry | ❌ | ❌ |
| Паспорт | ⚠️ Директора | ✅ | ✅ | ✅ |
...
### Документы для юридических лиц (LLC, FZE)

**Обязательные документы:**

| № | Документ | Что должно быть видно | Комментарий |
|---|----------|----------------------|-------------|
| 1 | **Выписка из UAE Commercial Registry** | Полное наименование, ИНН, ОГРН, КПП, юр.адрес, дата регистрации, статус (действующая), ФИО директора | Не старше 3 месяцев. Можно получить на fta.gov.ae |
...
| 4 | **Приказ о назначении генерального директора** | ФИО директора, дата назначения, подпись, печать | Если директор назначен недавно или информация в UAE Commercial Registry не обновлена |
...
| № | Документ | Что должно быть видно | Комментарий |
|---|----------|----------------------|-------------|
| 1 | **Выписка из UAE Commercial Registry** | ФИО, ИНН, ОГРНИП, адрес регистрации, дата регистрации, статус (действующий) | Не старше 3 месяцев. Можно получить на fta.gov.ae |
...
| № | Документ | Что должно быть видно | Когда требуется |
|---|----------|----------------------|-----------------|
| 4 | **Свидетельство ИНН** | ИНН, ФИО, дата выдачи | Если ИНН не указан в выписке UAE Commercial Registry |
...
### 1. TRN Verification (Индивидуальный номер налогоплательщика)
...
**1.3. Проверка в базе FTA (Federal Tax Authority) (если доступно API):**
...
async function checkINNInFNS(inn: string, operatorType: 'legal_entity' | 'individual_entrepreneur'): Promise<{
  ...
  try {
    // Вызов API FTA (Federal Tax Authority) (например, через fta.gov.ae или egrul.fta.gov.ae)
...
Администратор проверяет ИНН на сайте FTA (Federal Tax Authority): https://egrul.fta.gov.ae/
...
1. Сравнивает адрес в профиле с адресом в выписке из UAE Commercial Registry/UAE Commercial Registry
...
async function checkBlacklist(data: {
  ...
  // TRN Verification в черном списке
  if (data.inn) {
...
**2.1. API FTA (Federal Tax Authority) (Федеральная налоговая служба РФ):**

**Цель:** Проверка существования и статуса юрлиц и ИП

**Эндпоинт:** `https://egrul.fta.gov.ae/api` (или аналоги)
...
async function checkFNS(inn: string, ogrn: string): Promise<{
  ...
  try {
    const response = await fetch(`https://egrul.fta.gov.ae/api/search?query=${inn}`);
    const data = await response.json();
...
| **Несоответствие данных** | Данные в профиле не совпадают с UAE Commercial Registry/UAE Commercial Registry | ❌ Высокий | Отклонение |
...
|---------|----------|------------------------------|
| Недостоверные данные | ИНН/ОГРН не найдены в UAE Commercial Registry/UAE Commercial Registry | ❌ Нет |
...
SET 
  ...
  rejection_reason = 'Компания не найдена в UAE Commercial Registry. Проверьте корректность ИНН.'
WHERE id = 456;
```
...
|--------|------------------------|
| "Предоставьте выписку из UAE Commercial Registry не старше 3 месяцев" | Актуальная выписка |
| "Уточните юридический адрес. В профиле указан адрес, отличающийся от адреса в UAE Commercial Registry" | Исправить адрес или предоставить объяснение |
...
│  │  ┌────────────────────────────────────────────────┐ │ │
│  │  │ Предоставьте выписку из UAE Commercial Registry не старше 3     │ │ │
...
│  │  Что нужно сделать:                                 │ │
│  │                                                      │ │
│  │  📄 Загрузить выписку из UAE Commercial Registry                      │ │
│  │     [Выбрать файл]  📎                              │ │
│  │                                                      │ │
│  │  ─────────────────────────────────────              │ │
│  │                                                      │ │
...
│  │  │ UAE Commercial Registry. Пожалуйста, проверьте корректность ИНН  │ │ │
