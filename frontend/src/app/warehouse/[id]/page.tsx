...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  try {
    const warehouse = await warehousesApi.getById(params.id);

    return {
      title: `${warehouse.name} - Storage in ${warehouse.emirate} | StorageCompare.ae`,
      description: warehouse.description || `Find storage at ${warehouse.name} in ${warehouse.emirate}. View available units, pricing, and reviews.`,
      openGraph: {
        title: warehouse.name,
        description: warehouse.description || `Storage facility in ${warehouse.emirate}`,
        type: 'website',
      },
    };
  } catch (error) {
    return {
      title: 'Warehouse Not Found | StorageCompare.ae',
    };
  }
}
...
export default async function WarehousePage({ params }: PageProps) {
  ...
  try {
    [warehouse, boxes, reviews] = await Promise.all([
      warehousesApi.getById(params.id),
      warehousesApi.getBoxes(params.id),
      warehousesApi.getReviews(params.id),
    ]);
  } catch (error) {
    notFound();
  }
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {warehouse.features.map((feature, idx) => (
                        <span
                          key={idx}
                          className="inline-block px-3 py-1 text-sm font-medium text-gray-700 bg-blue-50 rounded-full"
                        >
                          {feature}
                        </span>
                      ))}
